...
| `-n N` | `--limit N` | N 件取得したら停止（`0` = 無制限、デフォルト: `0`） |
| | `--exclude PAT` | PAT（グロブ）にマッチするエントリを除外。複数回指定可 |
...
# 件数制限
gf '*.log' -n 5                  # 最初の5件だけ取得
gf -n 1 'main.go'                # 最初のマッチで停止

# 除外
gf '*.go' --exclude '*_test*'              # テストファイルを除外
gf --exclude node_modules --exclude dist   # ビルドディレクトリを除外
gf '*.ts' --exclude '*.d.ts' ./src        # 型定義ファイルを除外
```
